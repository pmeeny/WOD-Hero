...
if ( !function_exists('wp_ajt_user_notification') ) {
    function wp_social_user_notification( $user_id, $plaintext_pass = '',$register_type='') {
        ...
        if($register_type == 'seller'){
            $admin_email = 'seller@ajtlearning.com';
        }
        else{
            $admin_email = get_option('admin_email');
        }

        $message  = sprintf(__('New user registration on your AJTLearning %s:'), get_option('blogname')) . "\r\n\r\n";
...
function trainerform(){
    ...
    //print_r($_POST);die;
    if($gym_name==''){
        respond_by_json(true,"Please select gym name.",'alert-danger');
    }
    else{
        if(wp_verify_nonce($securitycode,'trainercode') )
        {
            if (!is_wp_error($user_id)) {
                ...
                if($gym_name != ''){
                    ...
                    if($gym_name_last){
                        ...
                        add_user_meta($user_id, 'gym_name', $gym_name, true);
                    }
                }
                ...
                if($insta_link != ''){
                    ...
                    if($insta_last){
                        ...
                        add_user_meta($user_id, 'my_instagram', $insta_link, true);
                    }
                }
                ...
                if($fb_link != ''){
                    ...
                    if($fb_last){
                        ...
                        add_user_meta($user_id, 'my_facebook', $fb_link, true);
                    }
                }
                ...
                if($tw_link != ''){
                    ...
                    if($tw_last){
                        ...
                        add_user_meta($user_id, 'my_twitter', $tw_link, true);
                    }
                }

                respond_by_json(true,"Information added successful.",'alert-success');
            }
        }
        else
        {
...
add_action('wp_ajax_trainerform','trainerform');
        ...
        $personalBest = array();
            foreach($workout_results as $result){
                ...
                $html.='<tr>';
                    ...
                    if(!empty($times)){
                        $html.='<td>'.$times['text'].' '.$times['unit'].'</td>';
                    }
                    else{
...
function getLastPersonalBest(){
    ...
    $userId = get_current_user_id();
    if($userId != '') {
        $sql = "SELECT * FROM " . $wpdb->prefix . "add_workout WHERE user_id = " . $userId ." order by id desc limit 6 ";
        $workout_results = $wpdb->get_results($sql, ARRAY_A);
        ...
        if (!empty($personalBests)) {
            ...
            foreach ($personalBests as $result) {
                ...
                if (!empty($result['add_date'])) {
                    $html .= '<td>' . date('d M Y', $result['add_date']) . '</td>';
                }
                $html .= '</tr>';
...
function get_personal_best_for_graph_json_option($wk_id=0, $is_home = 1)
{
    global $wpdb;
    if($wk_id !='')
    {
        $current_workouts_details = $wpdb->get_results($wpdb->prepare("SELECT * FROM  " . $wpdb->prefix . "add_workout  WHERE 1=%d AND user_id = %d  AND publish=%d AND wk_name=%d", array(1,get_current_user_id(), 1, $wk_id)));
        $total_val = array();
        ...
        if($is_home) {
            ...
            if (!empty($weight)) {
                ...
                $weight_data = '';
                foreach ($total_val as $key => $val) {
                    $weight_data += is_array($val) ? array_sum($val) . ', ' : $val . ', ';
                }
            ...
            if (!empty($time)) {
                $time_arr['name'] = 'Time';
                $time_data = '';
                foreach ($total_time_val as $key => $val) {
                    $time_data += is_array($val) ? array_sum($val) . ', ' : $val . ', ';
                }
            ...
            if (!empty($distance)) {
                ...
                foreach ($total_distance_val as $key => $val) {
                    $distance_data += is_array($val) ? array_sum($val) . ', ' : $val . ', ';
                }
            ...
            $options_obj = array(
                'title' => array('text' => 'My personal bests', 'x' => -20),
                'xAxis' => array(
            ...
            $r = 0;
            if (!empty($weight_arr)) {
                $options_obj['series'][$r] = $weight_arr;
                $r++;
            }

            if (!empty($time_arr)) {
                $options_obj['series'][$r] = $time_arr;
                $r++;
            }

            if (!empty($distance_arr)) {
                $options_obj['series'][$r] = $distance_arr;
            }

            return json_encode($options_obj);
        ...
        else
        {
           ...
           ?>
            ...
            yAxis: {
                        ...
                        text:
                        '<?php
                                   if($weight){ echo 'Weight'; }elseif($time){ echo 'Minutes'; }elseif($distance){ echo 'Kilometer '; } ?>'
                        },
                        plotLines: [{
                                        value: 0,
                                        width: 1,
            ...
            series: [<?php
                ...
                {
                name:'Weight',
                data: [<?php foreach($total_val as $key=>$val){ echo is_array($val) ? array_sum($val).', ':$val .', ';  } ?>]
                },
            <?php
                      }
                        if(!empty($time))
                ...
                {
                name:'Time',
                data: [<?php foreach($total_time_val as $key=>$val){ echo is_array($val) ? array_sum($val).', ':$val .', ';  }?>]
                },
            <?php
                       }
                ...
                {
                name:'Distance',
                data: [<?php foreach($total_distance_val as $key=>$val){ echo is_array($val) ? array_sum($val).', ':$val .', ';  }  ?>]
                },
            <?php
                     }
            ?>
...
function get_personal_best_for_graph(){
    global $wpdb;
    if(!empty($_POST['wid'])) {
        echo get_personal_best_for_graph_json_option($_POST['wid']);
        exit();
    }
}
...
function wod_hero_action_callback()
{
        ...
        if( empty($first_name) || empty($profile_picture[0]))
        {
            wp_redirect( site_url('/settings/').'?error='.urlencode('Sorry, verify your field value.') );   exit();
        }


        update_user_meta( $current_user_detail->ID, 'gender', $_POST['gender'] );
        $last_name = $_POST['last_name'];
        wp_update_user( array( 'ID' => $current_user_detail->ID, 'first_name' => $first_name, 'last_name'=>$last_name ) );
        wp_redirect( site_url('/settings/').'?msg='.urlencode('Successfully update!') );   exit();
