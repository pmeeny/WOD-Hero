...
function get_personal_best_for_graph_json_option( $wk_id = 0 )
{
    global $wpdb;
    if($wk_id !='')
    {
...
DATE_ADD(CURDATE(), INTERVAL -7 DAY)", array(1,get_current_user_id(), 1, $wk_id)));
        ...
        foreach ($current_workouts_details as $key => $val) {
            ...
            if (!empty($total_times->text)) {

                $time_chunk = explode(':', $total_times->text);

                $time = true;

                    if(!empty($time_chunk[0]) || !empty($time_chunk[1]) || !empty($time_chunk[2]))
                    {
                        $total_time_val[trim($new_key)][$key] = floatval(($time_chunk[0]) / 60) + floatval(($time_chunk[2]) * 60) + floatval($time_chunk[3]);
                     }
            }
            ...
            if (!empty( $val->reps )) {
                $reps = true;
                $total_reps_val[trim($new_key)][$key] = $val->reps;
            }
            ...
            $rep_arr = array();
            if (!empty($total_reps_val)) {
                $total_reps_arr['name'] = 'Repeat';
                $total_reps_data = array();
                foreach ($total_reps_val as $key => $val) {
                    $total_reps_data[] = is_array($val) ? array_sum($val) : $val ;
                }
                $total_reps_arr['data'] = $distance_data;
            ...
            //$yAxis_title = rtrim(' / ',$yAxis_title);


            $data = array('graph_option'=>$options_obj,'yAxis_title'=>array('text'=>$yAxis_title) );
...
add_action( 'wp_ajax_nopriv_wod_hero', 'wod_hero_action_callback' );
...
function wod_hero_action_callback()
{
    ...
    if (wp_verify_nonce($wod_hero_user_info_security_code, 'wod_hero_user_info_code'))
    {
        ...
        if (empty($first_name) || empty($profile_picture)) {
        ...
        $display_name_arr = exploade(' ',$_POST['display_name']);
